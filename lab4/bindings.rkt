...
(define (compute-length get-line-segment get-start-point get-end-point)
  (let* ([s (get-line-segment)]
    [start (get-start-point s)]
    [end (get-end-point s)])
    (sqrt (+ (sqr (- (car start) (car end))) (sqr (- (cdr start) (cdr end)))))))
...
(define (distance x y)
  (compute-length (λ() 0) (λ(a) x) (λ(a) y)))
...
(define (compute-perimeter points)
  (define (f L sum) (if (= 1 (length L))
             sum
             (f (cdr L) (+ sum (distance (car L) (cadr L))))))
  (f (append points (list (car points))) 0))
