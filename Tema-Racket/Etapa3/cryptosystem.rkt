...
(define (key n)
  (let ([e (cadr (get-nth-quadruple n))]
        ...
        [L_f (list a1 b1 c1 a2 b2 c2 a3 b3 c3)])
    (map (λ(x) (modulo x 27)) (map (λ(functions) (functions e f)) L_f))))
...
(define (message->codes message)
  (let ([M (string->list message)])
    (map (λ(x) (if (= x -64) (* x 0) (* x 1))) (map (λ(c) (- (char->integer c) 96)) M)) ))
...
(define (codes->message codes)
    (list->string (map (λ(x) (if (= x 0) (integer->char (+ x 32)) (integer->char (+ x 96)))) codes) ))
...
(define (extend-key key size)
  (let iterate ([L '()]
    ...
    (cond [(= (length L) size) L]
          [(null? K) (iterate L key)]
          [else  (iterate (append L (list (car K))) (cdr K))])))
...
(define (codes-ed function message key)
  (foldl (λ(x y acc) (append acc (list(modulo (function x y) 27)))) '() message key))

(define (encrypt-codes message key)
  (codes-ed + message (extend-key key (length message))))
...
(define (message-ed function m k)
  (let ([message (message->codes m)]
        [key (extend-key k (length (string->list m)))])
  (foldl (λ(x y acc) (append acc (list(modulo (function x y) 27)))) '() message key)))

(define (encrypt-message message key)
  (codes->message (message-ed + message key)))
